...
exports.check = function (value, field) {
    var type = field['Type_Struct'];
    var canNull = field['Null'];
    ...
    if((value===undefined||value===null)&&canNull=='NO'){
        if((defaultValue===null||defaultValue===undefined)&&(String(extra).trim() === '' || extra === null || extra === undefined)){
            return new Error('table:'+tableName+' field:'+fieldName+' value:null error:can not be null');
    ...
    var returnVal = func(value, type);
    if(returnVal&&returnVal.message){
        returnVal = new Error('table:'+tableName+' field:'+fieldName+' value:'+value+' error:'+returnVal.message);
    }
    return returnVal;
}
